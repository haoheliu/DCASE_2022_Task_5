...
if(not os.path.exists("Development_Set")):
    os.system("unzip Development_Set.zip")
...
class Feature_Extractor:
    ...
    def spectral_rolloff(self, S, roll_percent=0.9):
        return librosa.feature.spectral_rolloff(S=S, sr=self.sr, roll_percent=roll_percent)
    ...
    def zero_crossing_rate(self, y):
        assert np.max(y) <= 1
        return librosa.feature.zero_crossing_rate(y, frame_length=self.n_fft, hop_length=self.hop)
    ...
    def visualize_result(self, result):
        for k in result.keys():
            if(result[k].shape[1] == 1): self.draw_plot(result[k], name=k)
            elif(result[k].shape[1] > 1): self.draw_spec(result[k], name=k)
    ...
    def extract_feature(self, audio_path):
        ...
        result["spec"], result["phase"] = librosa.magphase(librosa.stft(result["waveform"], hop_length=256, n_fft=1024))
...
def process(fnames):
    ...
    for file in tqdm(fnames):
        # if(os.path.exists(file.replace(".wav","_%s.npy" % "pcen"))): continue
        try:
...
# Normalize
for k in features:
    if("un_normalized" in features): continue
    print(k)
    array_list = []
