...
def read_result(folder):
    ...
    if(len(json_file) < 1): return None
    ret = []
    for each in json_file:
        # json_file = json_file[0]
        dic = read(each)['scores_per_subset']
        if("PB" not in dic.keys()): return None
        ret.append(dic['PB']['f-measure'])
    return ret

result = {}
i = 0
for each in glob(os.path.join(path,"*/%s" % folder)) + glob(os.path.join(path,"*/*/%s" % folder))+glob(os.path.join(path,"*/*/*/%s" % folder))+glob(os.path.join(path,"*/*/*/*/%s" % folder)) + glob(os.path.join(path,"*/*/*/*/*/%s" % folder)) + glob(os.path.join(path,"*/*/*/*/*/*/%s" % folder)):
    ...
    for folder in os.listdir(dirname):
        ...
        if(isfloat(folder)):
            if(folder not in res_dict.keys()): res_dict[folder] = []
