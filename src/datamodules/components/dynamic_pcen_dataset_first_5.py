...
class PrototypeDynamicArrayDataSetWithEval(Dataset):
    def __init__(self, path: dict = {}, features: dict = {}, train_param: dict = {}):
        ...
        """
        print("Building training dataset. Use the first 5 samples from the evaluation set for training")
        self.path = path
        ...
        self.all_train_csv_files, self.all_eval_csv_files, self.extra_csv_files = self.get_all_csv_files()
        self.all_csv_files = self.all_train_csv_files + self.all_eval_csv_files + self.extra_csv_files
        self.train_classes = []
        self.eval_classes = []
        ...
        self.train_eval_class_idxs = [self.classes2int[x] for x in self.train_classes + self.eval_classes if(x in self.classes)]
        self.extra_train_class_idxs = [self.classes2int[x] for x in self.extra_train_classes if(x in self.classes)]
    ...
    def load_mask(self):
        ...
        for clss in tqdm(self.meta.keys()):
            ...
            if(not os.path.exists(os.path.join(self.path.mask_dir, clss))): self.build_mask()
            
            for file in os.listdir(os.path.join(self.path.mask_dir, clss)):
                if(".npy" not in file): continue
                self.mask[clss].append(np.load(os.path.join(self.path.mask_dir, clss, file)))
            
    def build_mask(self):
        for file in tqdm(self.all_csv_files):    
            audio_path = file.replace("csv", "wav")
            if audio_path not in self.pcen.keys():
                self.mel[audio_path] = self.fe.extract_feature(audio_path, feature_types=['mel'], normalized=False)
        ...
        if(class_name in self.extra_train_classes):
            ...
            if(class_name not in self.segment_buffer.keys()):
                ...
                if(end-start > self.segment_level_training_length):
                    start = np.random.uniform(0,end-self.segment_level_training_length)
    ...
    def concate_mask(self, x, mask):
        ...
        return np.concatenate([x[None,...], mask[None,...]], axis=0) # concatenate on the channel dimension
    ...
    def update_meta(self, start_time, end_time, cls_list, csv_file):
        ...
        for start, end, clss in zip(start_time, end_time, cls_list):
            ...
            if(csv_file in self.extra_csv_files and clss not in self.extra_train_classes):
            ...
            neg_start, neg_end = np.clip(self.meta[clss]["neg_start_time"]-0.025, a_min=0, a_max=None), np.clip(start+0.025, a_min=None, a_max=self.meta[clss]["total_audio_duration"][-1])
    ...
    def get_all_csv_files(self):
            ...
            return [
                ...
                for file in glob(os.path.join(path_dir, extension))
            ] if (self.path.extra_train_dir is not None) else []
    ...
    def get_freq_mask(self, info_prev, info, info_next, mel):
        ...
        if(neg_length_prev is None): neg_length_prev = current_pos_freq
        if(neg_length_next is None): neg_length_next = current_pos_freq
    ...
    def build_freq_energy_mask(self):
        ...
        for clss in tqdm(self.meta.keys()):
            ...
            if(len(os.listdir(os.path.join(self.path.mask_dir, clss))) != 0): continue
            print(clss)
            for i, (s, e) in tqdm(enumerate(self.meta[clss]['info'])):
                ...
                if(i == 0): 
                    info_prev = (0.0,0.0)
                    info_next = self.meta[clss]['info'][i+1] if(len(self.meta[clss]['info']) > 1) else self.meta[clss]['info'][0]
                ...
                self.meta[clss]['freq_mask'].append(self.get_freq_mask(info_prev, (s,e), info_next, mel.T))
            ...
            for i, each in enumerate(self.meta[clss]['freq_mask']):
                ...
                os.makedirs(os.path.join(self.path.mask_dir, clss), exist_ok=True)
                plt.plot(each)
                plt.savefig(os.path.join(self.path.mask_dir, "%s/visualize_%s.png" % (clss, i)))
                np.save(os.path.join(self.path.mask_dir,"%s/mask_%s.npy" % (clss, i)), each)
                plt.close()
