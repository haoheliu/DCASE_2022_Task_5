...
class PrototypeDynamicArrayDataSetVal(Dataset):
    def __init__(self, path: dict = {}, features: dict = {}, train_param: dict = {}):
        ...
        print("Build the validation dataset with suffix", self.features.feature_types.split("@"))
        ...
        self.eval_class_idxs = [self.classes2int[x] for x in self.eval_classes if(x in self.classes)]
    ...
    def load_mask(self):
        for clss in self.meta.keys():
            self.mask[clss] = np.load(os.path.join(self.path.mask_dir, "mask_%s.npy" % clss))
    ...
    def get_freq_mask(self, info_prev, info, info_next, mel):
        ...
        return identify_changed_freq(current_pos_freq, neg_length_prev, neg_length_next)[None,...]
