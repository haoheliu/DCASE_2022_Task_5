...
def prototypical_loss(_input, target, n_support):
    ...
    dists = euclidean_dist(query_samples, prototypes) # torch.Size([50, 512]), torch.Size([10, 512])
    # dists = cosine_dist(query_samples, prototypes) # torch.Size([50, 512]), torch.Size([10, 512])
    log_p_y = F.log_softmax(-dists, dim=1).view(n_classes, n_query, -1) # torch.Size([10, 5, 10]) (how many class, how many queries, how many target classes)
...
def prototypical_loss_filter_negative(_input, target, n_support):
    ...
    dists = euclidean_dist(query_samples, prototypes) # torch.Size([50, 512]), torch.Size([10, 512])
    # dists = cosine_dist(query_samples, prototypes) # torch.Size([50, 512]), torch.Size([10, 512])
    log_p_y = F.log_softmax(-dists, dim=1).view(n_classes // 2, n_query, -1) # torch.Size([10, 5, 10]) (how many positive class, how many queries, how many target classes)
